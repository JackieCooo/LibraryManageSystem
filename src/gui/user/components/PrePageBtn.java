...
public class PrePageBtn extends JButton implements ParentAvailable<TopPanel>, Updatable {
    ...
    public void stateUpdate() {
        this.setEnabled(!Global.prePage.empty());
    }
    ...
    private void setupListener(){
        this.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (!Global.prePage.empty()) {
                    MainPanel p = getParentPanel().getParentPanel().getMainWindow();
                    p.getComponent(Global.curPage).setVisible(false);
                    p.getComponent(Global.prePage.peek()).setVisible(true);
                    Global.nexPage.push(Global.curPage);
                    Global.curPage = Global.prePage.peek();
                    Global.prePage.pop();
                }
                stateUpdate();
